...
// 2026-05-30: зум при ВЫБОРЕ парковки (клик по зоне на карте или в списке).
// Единственный порог z: клик приближает карту ДО этого уровня. Приближение
// работает только «внутрь» — если карта уже ближе этого z, зум не меняется
// (карта лишь центрируется на парковке), отдаления не происходит.
// Хочешь, чтобы зум срабатывал с более близкого старта / приближал сильнее —
// подними это число; мягче — опусти. Предел ymaps3 — ~21.
